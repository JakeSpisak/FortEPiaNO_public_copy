...
if True:
	...
	finalizePlot(
		...
		x_T=True,
		Neff_axes=True,
		tightrect=(-0.025, -0.035, 1.02, 1.03),
		)
...
for ol, fn in [
		...
		for i in range(4):
			for j in range(i+1, 4):
				try:
					...
					pass
	ax.yaxis.set_major_formatter(FuncFormatter(lambda x, pos: '{0:g}'.format(x*1e4)))
	finalizePlot(
		"plots/3p1/rho_offdiag_%s.pdf"%fn,
		ylab=r"$10^4\varrho_{\alpha\beta}$",
		legend=False,
		x_T=True,
		)
	...
	ax = plt.gca()
	ax.add_artist(first_legend)
	ax.yaxis.set_major_formatter(FuncFormatter(lambda x, pos: '{0:g}'.format(x*1e2)))
	for io, o in enumerate(ol):
		plt.plot(np.nan, ls=styles[io], color='k', label=o.label)
	hands, labs = ax.get_legend_handles_labels()
	second_legend = plt.legend(hands[-2:], labs[-2:], loc='lower right')
	...
	finalizePlot(
		...
		ylab=r"$10^2\frac{d\varrho_{\alpha\beta}}{dx}$",
		legend=False,
		x_T=True,
		)
...
if True:
	...
	a3nu.plotFD(lab=r"$3+1$ DW", ls=":", lc=colors[3], rescale=Ue4l.wfin, fac=(Ue4l.Neff-a3nu.Neff))
	a3nu.plotFD(lab="", ls=":", lc=colors[3], rescale=Um4l.wfin, fac=(Um4l.Neff-a3nu.Neff))
	first_legend = plt.legend(loc=(0.05, 0.05))
	ax = plt.gca()
	ax.add_artist(first_legend)
	for fn, cl, ol in [
			["ord", (0, 4, 5, 3), (Ue4h, Ue4l, Um4l, a3nu)],
			]:
		for o, c in zip(ol, cl):
			plt.plot(np.nan, ls='-', color=colors[c], label=o.label)
			for j, i in enumerate([0, 3]):
				try:
					o.plotRhoFin(i, ls=styles[j], lc=colors[c], y2=True, lab="")
				except IndexError:
					pass
		hands, labs = ax.get_legend_handles_labels()
		second_legend = plt.legend(hands[-4:], labs[-4:], loc='upper right')
		finalizePlot(
			"plots/3p1/rhofin_diag_%s.pdf"%fn,
			legend=False,
			xlim=[-0.2, 15],
			ylim=[1e-4, 1],
			ylab=r"$y^2\varrho_{\alpha\alpha}^{\rm fin}(y)$",
			xscale="linear",
			yscale="log",
			)
