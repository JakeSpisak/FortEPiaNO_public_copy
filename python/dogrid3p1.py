...
def sortFiles(files, from_heaviest):
    """Sort the list of files that correspond to a given 3+1 grid.
    If requested, order them according to the mass splitting.

    Parameters:
        files: the initial list of files
        from_heaviest: if True, sort the file names
            according to the mass splitting, starting from the heaviest

    Output:
        the reordered list of files
    """
    return (
        sorted(
            files,
...
def safegetattr(obj, attr, default):
    """Safe version of getattr, which returns a default value
    if the attribute does not exist

    Parameters:
        obj: any python object
        attr: the requested attribute name
        default: the value to return if the attribute does not exist

    Output: the requested attribute or the default value
    """
    try:
...
def setParser():
    """Prepare a parser for the command line arguments.
    Add all possible subcommands and options
...
def fillGrid(args):
    """Create a list of points that contains all the grid points to run
    with FortEPiaNO.
    It will create a regular grid unless args.ternary is set and True,
    in which case the list is generated by ternary.helpers.simplex_iterator
    for each step in the mass splitting

    Parameters:
        args: a Namespace

    Output:
        a multidimensional np.ndarray
    """
...
def write_grid_cfg(args):
    """Save the grid configuration (ranges, number of points)
    into a params.cfg file
...
def read_grid_cfg(gridname):
    """Read a params.cfg grid file and recompute the list of grid points
    with a call to fillGrid

    Parameters:
        gridname: the name of the grid, to be used to obtain the folder
            where it is stored.

    Output:
        a tuple with two elements:
            the dictionary of configuration values
            the output of fillGrid (the np.ndarray of grid points)
    """
...
def contourplot(
    ...
    """Produce a contour plot with the given settings

    Parameters:
        xv, yv: the x and y points (1D arrays)
        points: the z points to use in the countour plot
            (it should be possible to reshape it to (len(yv), len(xv))
        fname (default None): if not None, the file name
            where to save the produced figure
        levels (default [3.0, 3.1, 3.3, 3.5, 3.7, 3.9]):
            a list of levels to use in contourf
        title (default None): a title for the figure
        xlab (default None): label for the x axis
        ylab (default None): label for the y axis
        xlim (default None): limits for the x axis
        ylim (default None): limits for the y axis
        bfx (default -1): the x coordinate of the best fit point
            (a marker will be added in correspondence
            if both the coordinates are positive)
        bfy (default -1): the y coordinate of the best fit point
            (a marker will be added in correspondence
            if both the coordinates are positive)
        bfup (default False): if True, use a \leftarrow to indicate the
            best fit point (useful if it lies out of the axes limits)
        lsn_contours (default []): list of file names with contours
            of various kind (oscillation experiments, for example)
            to be loaded and added to the plot
        lsn_contours_convssq2th (default True): if True, assume that
            the LSN contours are saved in terms of $\\sin^2 2\\theta$
            and must be converted in terms of $\\sin^2 \\theta$
        colorbar (default True): if True, add a colorbar to the plot
        colorbar_fname (default None): if not None, the name of the file
            where to save the color bar alone
        textbox (default None): if not None, the text for a text box
            to be added in the upper right corner
        cbarlabel (default r"$N_{\rm eff}$"): the label
            for the y axis of the color bar
        tightrect [default (-0.03, -0.05, 1.05, 1.02)]:
            a rect for tight_layout
    """
    try:
...
def call_fill(args):
    """Action for the "fill" command line subcommand.
    Function that fills the Neff values in the points of the grid
    where it is known that Neff will be either 3.04 or 4.05
    because for higher (smaller) migins angles or mass splittings
    it is already at that level.
    e.g.: if Neff=4.05 for some $\\sin^2\\theta=0.1$,
    it is also valid that Neff=4.05 for $\\sin^2\\theta\geq0.1$.
    The combinations to fill are searched in each direction
    ...
    Output:
        a tuple with two elements as in read_grid_cfg
    """
...
def call_plot(args, gridContent=None):
    """Action for the "plot" command line subcommand.
    Read a grid, if necessary, then do contour plots using two relevant
    parameters that are varying in the grid.
    Convert the angles appropriately if requested.

    Parameters:
        args: a Namespace
        gridContent: if not None, a tuple with the output of call_read
    """

    def convert_grid(convert_x, convert_y, pts):
        """Convert the x and y arrays if they are $\\sin^2 2\theta$
        and one wants $\\sin^2\\theta$, or $|U_{a4}|^2$

        Parameters:
            convert_x, convert_y: the names of the x and y parameters
            pts: the list of points to treat and convert
                (includes all the grid parameters)

        Output:
            the array of converted points
        """
...
def call_prepare(args):
    """Action for the "prepare" command line subcommand.
    It prepares the grid folders, removes old files
    and produces all the new ini files
    ...
    if args.gridname in ["plot", "ternary"]:
        print(
            "Sorry, I cannot let you name your grid 'plot' or 'ternary', "
            + "otherwise I will have problems in the future."
        )
        return
...
def call_read(args):
    """Action for the "read" command line subcommand.
    Open the grid configuration file, then for each grid point
    try to check and read the folder content.
    If the folder exists and the run has completed,
    read it, add it as a point available for plotting.
    ...
    Output:
        a tuple with three elements:
            a dictionary with the grid configuration
            a np.ndarray with the grid points
            a list of FortEPiaNORun instances for the existing points
    """
...
def call_run(args):
    """Action for the "run" command line subcommand.
    Reads a list of ini files, and calls bin/fortepiano
    for each ini file in the selected range.
    If running in local mode, it directly calls bin/fortepiano,
    once for each ini file, sequentially.
    If running in cluster mode, it calls clusterlauncher
    (this is what we use in the theory cluster in Valencia,
    but it can be adapted for other cases!).
    Reports the number of existing and submitted ini files at the end.
...
def call_ternary(args, gridContent=None):
    """Action for the "ternary" command line subcommand.
    Produce ternary plots for the requested grid and save it to a file.
