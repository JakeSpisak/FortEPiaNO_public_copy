...
def runConfig(
    args, values, logname="", verbose=0, low_reheating=False, no_interpolation=False
):
    nnu = values["nnu"]
    ...
    if low_reheating:
        if no_interpolation:
            raise ValueError("Cannot use interpolations with Low-Reheating!")
        fpw.pass_low_reheating(
            args.Trh,  # Trh
        )
        fpw.w_set_low_reheating()
