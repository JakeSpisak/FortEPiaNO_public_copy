module fpOutput
	...
	subroutine saveRelevantInfo(x, vec)
		...
		if (save_Neff) then
			neff = Neff_from_rho_z(vec(ixz)+1.d0)
...
#ifdef LOW_REHEATING
			write(iu, multidblfmt) &
				x, totalRadiationDensity(x,z), totalRadiationDensity(x,z)+x*vec(ixphi)
#else
			write(iu, multidblfmt) &
	...
	subroutine store_headers
		...
		if (save_z_evolution) then
			call openFile(iu, trim(outputFolder)//'/z_header.dat', firstWrite)
			if (save_w_evolution) then
				write(iu, "(*(A17))") "x", &
#ifdef LOW_REHEATING
					"t", &
	...
	subroutine finalresults
		...
		w = nuDensVec(ixw) + 1.d0
		z = nuDensVec(ixz) + 1.d0
