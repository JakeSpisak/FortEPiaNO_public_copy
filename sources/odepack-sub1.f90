...
!*DECK DIPREP
      ...
      integer,allocatable,dimension(:):: iwork ! ((( PABLO ))) added
      integer ERROR ! ((( PABLO ))) added
      INTEGER IOWND, IOWNS, &
         ICF, IERPJ, IERSL, JCUR, JSTART, KFLAG, L, &
         LYH, LEWT, LACOR, LSAVF, LWM, LIWM, METH, MITER, &
         MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
...
!C Call DPREP to do matrix preprocessing operations. --------------------
      allocate(iwork(lwm),STAT=ERROR) ! ((( PABLO ))) added
        IF (ERROR.NE.0) STOP 'NO HAY MEMORIA SUFICIENTE' ! ((( PABLO ))) added
      iwork=int(rwork(lwm)) ! ((( PABLO ))) added
      CALL DPREP (NEQ, Y, RWORK(LYH), RWORK(LSAVF), RWORK(LEWT), &
!         RWORK(LACOR), IA, JA, RWORK(LWM), RWORK(LWM), IPFLAG, F, JAC) ! ((( PABLO ))) converted to:
         RWORK(LACOR), IA, JA, RWORK(LWM), iWORK(LWM), IPFLAG, F, JAC) ! ((( PABLO ))) (rwork to iwork)
...
!         RWORK(LACOR), IA, JA, IC, JC, RWORK(LWM), RWORK(LWM), IPFLAG, & ! ((( PABLO ))) converted to:
         RWORK(LACOR), IA, JA, IC, JC, RWORK(LWM), iWORK(LWM), IPFLAG, & ! ((( PABLO ))) (rwork(lwm) to iwork(lwm))
         RES, JAC, ADDA)
      LENWK = MAX(LREQ,LWMIN)
      IF (IPFLAG .LT. 0) RETURN
