module fpEquations
	...
	subroutine drhoPhi_o_dx(x, z, rhoPhi, ydot, n)
		...
		aux = x*rhoPhi
		ydot(ixphi) = -aux*GammaPhi*overallFactor / (m_e*m_e*sqrt(totalRadiationDensity(x,z) + aux))
...
#endif
#ifdef LOW_REHEATING
		lrh_contr = x*ydot(ixphi) / (2.d0*z**3)
		num=num - lrh_contr
		numw=numw - lrh_contr
#endif
...
#ifdef LOW_REHEATING
			tx = getNewTx(tx, xend, xend - xs, totalRadiationDensity(xend, nuDensVec(ixz)+1.d0), nuDensVec(ixphi))
