...
\label{ssec:nsi}
In case NSI are active (the code must be compiled with \texttt{FULL\_F\_AB=1}),
...
\epsilon^L_{e\mu} & \tilde g_L + \epsilon^L_{\mu\mu} & \epsilon^L_{e\mu} &\ldots \\
...
\epsilon^R_{e\mu} & g_R + \epsilon^R_{\mu\mu} & \epsilon^R_{e\mu} &\ldots \\
