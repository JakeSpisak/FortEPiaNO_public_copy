...
Unfortunately, the current implementation of the Gauss--Laguerre method suffers a small non-conservation
of the neutrino number density,
that makes the results less precise than when using a linear spacing
of the nodes.
...
(Gauss--Laguerre integration, $N_y=50$, tolerance $10^{-6}$).
...
When considering a linear spacing of the $y_i$ nodes,
the instability is even smaller than when using the Gauss--Laguerre method.
For $N_y>60$, the final \Neff\ values are stable at the $\ll10^{-4}$ level.
...
using a much denser momentum grid.
%
\item
