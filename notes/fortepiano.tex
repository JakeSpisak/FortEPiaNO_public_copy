...
\qquad
\mathbb{E}_\nu=G_S\frac{1}{\pi^2}\left(\int \mathrm{d}y y^3\varrho\right) G_S\,
\quad\mbox{with }G_S=\text{diag}(1,1,1,0,\ldots)\,,
\end{equation}
...
Finally, the functions that define the phase space factors in the collision terms are \cite{Sigl:1992fn,deSalas:2016ztq,Bennett:2020zkv}:
\begin{eqnarray}
...
F_{\nu\nu}\left(\varrho^{(1)}, \varrho^{(2)}, \varrho^{(3)}, \varrho^{(4)}\right)
&=&
\left(1-\varrho^{(1)}\right)G_S\varrho^{(3)}G_S
\left[\left(1-\varrho^{(2)}\right)G_S\varrho^{(4)}G_S+\mathrm{Tr}(\cdots)\right]
\nonumber\\
&-&
\varrho^{(1)}G_S\left(1-\varrho^{(3)}\right)G_S
\left[\varrho^{(2)}G_S\left(1-\varrho^{(4)}\right)G_S+\mathrm{Tr}(\cdots)\right]
+\mathrm{h.c.}
\label{eq:F_nunu}\\
F_{\nu\bar\nu}\left(\varrho^{(1)}, \varrho^{(2)}, \varrho^{(3)}, \varrho^{(4)}\right)
&=&
\left(1-\varrho^{(1)}\right)G_S\left(1-\varrho^{(2)}\right)G_S
\left[\varrho^{(4)}G_S\varrho^{(3)}G_S+\mathrm{Tr}(\cdots)\right]
\nonumber\\
&-&
\varrho^{(1)}G_S\varrho^{(2)}G_S
\left[\left(1-\varrho^{(4)}\right)G_S\left(1-\varrho^{(3)}\right)G_S+\mathrm{Tr}(\cdots)\right]
\nonumber\\
&+&
\left(1-\varrho^{(1)}\right)G_S\varrho^{(3)}G_S
\left[\varrho^{(4)}G_S\left(1-\varrho^{(2)}\right)G_S+\mathrm{Tr}(\cdots)\right]
\nonumber\\
&-&
\varrho^{(1)}G_S\left(1-\varrho^{(3)}\right)G_S
\left[\left(1-\varrho^{(4)}\right)G_S\varrho^{(2)}G_S+\mathrm{Tr}(\cdots)\right]
% F_{\nu\bar\nu}\left(\varrho^{(1)}, \bar\varrho^{(2)}, \varrho^{(3)}, \bar\varrho^{(4)}\right)
% &=&
...
for later use of the results with the \texttt{PArthENoPE} code \cite{Pisanti:2007hk,Consiglio:2017pot,Gariazzo:2021iiu}.
...
$f_{\rm FD}(y_i)$.
%
\item \texttt{save\_intermediate\_steps}:
save the variables, their derivatives and the contributions from the different terms at all steps of the calculation (the output may take a lot of disk space).
%
...
%
\item \texttt{DO\_MUONS}: enable muon contributions to total energy density and matter potentials.
%
...
(for example, those for running \texttt{PArthENoPE})
